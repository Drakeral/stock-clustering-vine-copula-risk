...
The detailed pre-results definitions are frozen in `config/model_config.toml` and `docs/methodology_freeze.md`. Protocol amendment PA-001 authorizes the frozen contemporaneous Wikipedia universe/GICS snapshot, supported by Yahoo metadata and 2 January 2020 price-presence checks, as a provisional project foundation. It does not establish licensed provenance. The universe-provenance, data-construction, and modelling-readiness gates remain separate; all modelling and results under PA-001 must be labelled provisional research results. A later licensed reconciliation is required before any confirmatory provenance claim.
